# In Exercise 11.3 (b) on page 238, your task was to write a recur- sive R function to compute integer factorials, given some sup- plied non-negative integer x. Now, modify your function so that it throws an error (with an appropriate message) if x is negative. Test your new function responses by using the following:
...
# Note that solve throws an error if the supplied matrix can- not be inverted. With this in mind, write an R function that attempts to invert each matrix in a list, according to the follow- ing guidelines:
